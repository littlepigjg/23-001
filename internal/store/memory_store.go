...
// PanicGuardFn panic保护钩子函数类型
// 返回 true 表示应该 panic（模型不存在时），返回 false 表示静默返回 nil
type PanicGuardFn func(modelID model.ID) bool
...
// GetModelByIDWithGuard 根据ID获取设备型号（带guard版本）
// 当panicGuard存在且返回false时，模型不存在会返回(nil, nil)而非错误
func (s *MemoryStore) GetModelByIDWithGuard(_ context.Context, id model.ID) (*model.DeviceModel, error) {
	s.mu.RLock()
	defer s.mu.RUnlock()
