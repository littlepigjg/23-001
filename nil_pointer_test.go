...
func TestNilPointerDefect(t *testing.T) {
	...
	// Step 6: Set TotalDevices to non-zero so CalculateProgress will call calculateTimeBonus
	// This is needed because CalculateProgress returns 0 immediately if TotalDevices == 0
	task.TotalDevices = 1

	// Step 7: Test CalculateProgress on a new task
